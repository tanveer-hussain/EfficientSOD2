...
def run(input_path, output_path, model_path, model_type="dpt_hybrid", optimize=True):
    ...
    train_loader, training_set_size = get_loader(image_root, gt_root, depth_root, gray_root, batchsize=1, trainsize=352)
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        with torch.no_grad():
            for kk in range(images.shape[0]):
                img = images[kk,:,:,:].detach().cpu().numpy()
                pred_edge_kk = images[kk,:,:,:].unsqueeze(0)
                out = model.forward(pred_edge_kk)
                prediction = torch.nn.functional.interpolate(
                    out, size=pred_edge_kk.shape[:2], mode="bicubic", align_corners=False
                )
